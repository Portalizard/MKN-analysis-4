...
\begin{remark}
    \todo{написать настоящую теорему}

    Теорема Каратеодори: пусть $f:G \to D$, $f \in A(G)$, $f \in C(\overline{G})$, $f(\partial G) = \partial B$ --- однократное накрытие. Тогда для любого $w \in D$ равенство $f(z) = w$ имеет ровно одно решение, то есть $f(G) = B$ и $f$ --- однолистная.
\end{remark}

\todo{ковыряем автоморфизмы $\C^+$}.
...
\begin{remark}
    \todo{Рандомное замечание про порезанный диск}
\end{remark}
...
\begin{proof}
    ...
    Последовательность действий "<растянуть, отразить внутрь, сжать"> повторим счётное число раз. Мы получим $\varphi: B_{R_2}(0) \setminus \{0\} \to B_{R_2'}(0) \setminus \{0\}$. По лемме об устранимой особенности \ref{noness}, можно считать, что $\varphi: B_{R_2}(0) \to B_{R_2'}(0)$, при этом $\varphi$ --- конформное отображение, и \todo{$\varphi(0) = 0$}. Но, как известно, такими $\varphi$ могут быть только повороты, что невозможно, т.к. $R_2 \neq R_2'$.
\end{proof}
...
\begin{remark}
    ...
    \end{gather*}
    При $k \to 0$: $K \to const$, $K' \to +\infty$. При $k \to 1$: $K \to +\infty$, $K' \to const$. \todo{почему?}
\end{remark}
...
\begin{remark}
    ...
    \begin{enumerate}
        ...
        \item Если рассмотреть малую окрестность полюса, то окажется, что $f$ в этой окрестности однолистна:
        \begin{center}
        \begin{tikzpicture}
        ...
        \end{center}
        \todo{подробнее}.
        В окрестности полюса порядка $n$ функция ведёт себя как $c \cdot (z - z_0)^{-n}$. Но если $n \geq 2$, это означает, что функция не однолистна. Значит, полюса $f$ имеют порядок 1.
    \end{enumerate}
\end{remark}
